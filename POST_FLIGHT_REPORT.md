...
|---|---|---|---|---|---|
| `PROMPT_00_s.txt` | Execution contract + stack + mission constraints | `.prompts/PROMPT_00_s.txt`, repo-root RHOF workspace, runnable CLI/web, persisted sync path | Partial | Major contract elements are implemented (DB-backed sync, durable review resolution, dedup cluster persistence), DB-backed integration tests cover review resolution and migrate+sync idempotency, and scheduler execution now includes overlap prevention + retry/backoff. Remaining gaps are live fetching and deeper ops observability (`crates/rhof-adapters/src/lib.rs:72`, `crates/rhof-sync/src/lib.rs:537`, `crates/rhof-sync/src/lib.rs:1257`, `crates/rhof-web/src/lib.rs:875`). | Add lightweight CLI-wrapper smoke coverage in CI and continue scheduler observability/health improvements. |
...
| `PROMPT_05.txt` | Sync orchestration + reports + scheduler + idempotency | `rhof-sync`, DB persistence path, reports/parquet outputs, scheduler command path | Partial | Sync is DB-backed and idempotent for versions (`crates/rhof-sync/src/lib.rs:495`, `crates/rhof-sync/src/lib.rs:676`). Scheduler jobs now execute sync with overlap-skip and retry/backoff logging, but richer observability/health metrics are still minimal (`crates/rhof-sync/src/lib.rs:537`, `crates/rhof-sync/src/lib.rs:1257`). | Add scheduler metrics/health visibility (and optional persistent/distributed locking if unattended multi-instance deployment is in scope). |
...
- DB-backed integration coverage now also exists for migration + repeated sync idempotency in `rhof-sync` tests (`crates/rhof-sync/src/lib.rs:1753`) and is explicitly run in CI.
...
The repo is still fixture-first for production flows (live source fetching remains stubbed), so real-world source drift and parsing breakage are not yet exercised end-to-end despite strong local/CI workflows (`crates/rhof-adapters/src/lib.rs:72`, `crates/rhof-storage/src/lib.rs:207`).

### Single most leverage improvement to raise it fastest
Add one real live-fetch adapter path (even a narrow public HTML source) with an integration test, so the repo validates network fetch -> artifact -> parse under real source behavior instead of fixture-only flows (`crates/rhof-adapters/src/lib.rs:72`, `crates/rhof-storage/src/lib.rs:207`, `crates/rhof-sync/src/lib.rs:461`).
...
- Remaining weakness is continued reliance on fixture-first ingestion for real source evolution, plus limited scheduler observability/health reporting.
...
| PFN-016 | P3 | `PROMPT_05`, `PROMPT_09` | Scheduler mode now has overlap prevention and retry/backoff logging, but still lacks explicit health/metrics/status reporting (and no distributed lock story if multi-instance scheduling is ever desired). | `crates/rhof-sync/src/lib.rs:537`, `crates/rhof-sync/src/lib.rs:1257`, `crates/rhof-sync/src/lib.rs:1347` | Good local single-instance behavior, but weaker unattended operability visibility. | Add scheduler health/metrics logs (counters/last-success timestamp) and document single-instance assumption; add persistent lock only if multi-instance deployment becomes a goal. |
...
|---|---|---|---|---|
| 1 | Add a lightweight CLI smoke step in CI for `rhof-cli migrate` / `sync` | Protects docs-facing command wiring in addition to library-level tests | `crates/rhof-cli/src/main.rs:74`, `.github/workflows/ci.yml:65` | S |
| 2 | Add source-specific parser variant tests (multi-record/layout drift) | Strengthens raw parser resilience beyond current happy-path fixtures | `crates/rhof-adapters/src/lib.rs:361`, `crates/rhof-adapters/src/lib.rs:473`, `crates/rhof-adapters/src/lib.rs:1022` | M |
| 3 | Add scheduler health/metrics visibility (last-success, failures, skipped-overlap counters) | Improves unattended operability visibility now that retry/locking behavior exists | `crates/rhof-sync/src/lib.rs:537`, `crates/rhof-sync/src/lib.rs:1257` | S |
